...
# load chla model
mod.chl <- readRDS(file = "store/pChla.rds") # nla07_chl.R
...
## Distribution of coefficients for CH4 model
  coefs = MASS::mvrnorm(1, mu = coef(mod.ch4), Sigma = vcov(mod.ch4))
  
## Distribution of coefficients chla model
  coefs_chla = MASS::mvrnorm(1, mu = coef(mod.chl), Sigma = vcov(mod.chl))
  
  
  
## 2010 Data for iteration i
  dat_2010 <- datKR %>%
    rowwise() %>% # needed for rnorm
    mutate(
      # simulate uncertainty in Nvv and Pvv using model fit statistics
      # presented in Milstead 2013, Table 2, H6. Note that Nvv and Pvv
      # in datKR are in mg/L, same units used in Milstead. The chl and
      # GHG models require ug/L, however, and unit conversions are executed
      # below.
      Nvv2010_sim = rnorm(1, Nvv2010, sd = 0.21), 
      Pvv2010_sim = rtruncnorm(n = 1, a = 1.461529e-06, b = Inf, mean = Pvv2010, sd = 0.36)) %>% 
    select(Pvv2010, Pvv2010_sim, Nvv2010, Nvv2010_sim) %>%
    ggplot(aes(Pvv2010, Pvv2010_sim)) + geom_point() + scale_x_log10() + scale_y_log10()
    ungroup %>% # remove rowwise grouping
    mutate(
      # rename variable to be consistent with models
      `log10(Nvv)` = log10(Nvv2010 * 1000), # used in mod.chl. mg -> ug (units required for chl model)
      `log10(TP..ug.L.)` = log10(Pvv2010 * 1000), # used in mod.chl and mod.co2. mg -> ug (units required for chl and GHG model)
      `(Intercept)` = 1, # match names in mod.ch4 model
      # predict chla using Bryan's model trained on NLA07
      'log10(Chlorophyll.a..ug.L.)' = coefs_chla["(Intercept)"] + 
                                      (`log10(TP..ug.L.)` * coefs_chla["`log10(TP..ug.L.)`"]) +
                                      (`log10(Nvv)` * coefs_chla["`log10(Nvv)`"])) %>% # 
    select('(Intercept)','log10(Chlorophyll.a..ug.L.)') %>%
    as.matrix()
  
## TMDL Data for iteration i
dat_tmdl <- datKR %>%
    mutate(
      # rename variable to be consistent with models
      `log10(Nvv)` = log10(NvvTMDLnew), # used in mod.chl
      `log10(TP..ug.L.)` = log10(PvvTMDLnew), # used in mod.chl and mod.co2
      `(Intercept)` = 1, # match names in mod.ch4 model
      # predict chla using Bryan's model trained on NLA07
      'log10(Chlorophyll.a..ug.L.)' = coefs_chla["(Intercept)"] + 
                                      (`log10(TP..ug.L.)` * coefs_chla["`log10(TP..ug.L.)`"]) +
...
plot(dat.sf$geometry) #test, looks good
