...
```{r }
chl.p.summary <- do.call(cbind, lapply(chesDat %>% dplyr::select(Chla2010, Pin2010) %>% 
...
dat_2010 <-  dat %>%
  ...
  mutate('(Intercept)'=1,
         Chla2010 = log10(Chla2010 * 1000)) %>%
  rename('log10(Chlorophyll.a..ug.L.)'=Chla2010) %>%
...
dat_tmdl <-  dat %>%
  ...
  mutate('(Intercept)'=1,
         ChlaTMDLnew = log10(ChlaTMDLnew * 1000)) %>%
...
preds_2010_lakes %<>% mutate(preds_2010_lakes = pCh4(preds_2010_lakes[,3])) # transform according to functions above
preds_tmdl_lakes %<>% mutate(preds_tmdl_lakes = pCh4(preds_tmdl_lakes[,3])) # transform according to functions above
...
preds_2010_lakes %<>% mutate(preds_2010_lakes = preds_2010_lakes * (365 - 120), na.rm=TRUE) # 120 days of ice cover
preds_tmdl_lakes %<>% mutate(preds_tmdl_lakes = preds_tmdl_lakes * (365 - 120), na.rm=TRUE) # 120 days of ice cover
...
## JAKE, PLEASE FIX MY CRUDE TRANSFORMATIONS HERE, LEFT LIKE THIS SO YOU CAN SEE WHAT IS HAPPENING, BUT I'M PROBABLY WRONG...
})
summary(diff_ch4)
...
## 2010 Data
dat_2010 <-  dat %>%
  dplyr::select(Pin2010,AREASQKM) %>%
  mutate('(Intercept)'=1,
         AREASQKM = log10(AREASQKM),
         TP..ug.L. = log10(Pin2010 * 1000),
  ...
  dplyr::select(-Pin2010) %>%
  as.matrix()

## TMDL Data
dat_tmdl <-  dat %>%
  dplyr::select(PinTMDLnew,AREASQKM) %>%
  mutate('(Intercept)'=1,
         AREASQKM = log10(AREASQKM),
         TP..ug.L. = log10(PinTMDLnew * 1000),
         'log10(Surface.Area..km2.):log10(TP..ug.L.)' = AREASQKM*TP..ug.L.) %>%
  rename('log10(Surface.Area..km2.)'=AREASQKM,'log10(TP..ug.L.)'=TP..ug.L.) %>%
  relocate('(Intercept)','log10(Surface.Area..km2.)','log10(TP..ug.L.)','log10(Surface.Area..km2.):log10(TP..ug.L.)') %>%
  dplyr::select(-PinTMDLnew) %>%
  as.matrix()
...
preds_2010_lakes %<>% mutate(preds_2010_lakes = pCo2(preds_2010_lakes[,3])) # transform according to functions above
preds_tmdl_lakes %<>% mutate(preds_tmdl_lakes = pCo2(preds_tmdl_lakes[,3])) # transform according to functions above
...
preds_2010_lakes %<>% mutate(preds_2010_lakes = pN2o(preds_2010_lakes[,3]),) # transform according to functions above
preds_tmdl_lakes %<>% mutate(preds_tmdl_lakes = pN2o(preds_tmdl_lakes[,3])) # transform according to functions above
