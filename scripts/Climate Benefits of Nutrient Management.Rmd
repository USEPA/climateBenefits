...
```{r}
dat.yr <- dat %>%
  dplyr::select(WB_ID, day_of_ice_filled, contains("lk.d")) %>%
  mutate_at(vars(-WB_ID, -day_of_ice_filled), function(x) x * (365 - .$day_of_ice_filled)) %>% # ice cover duration winter 2019-2020
  rename_at(vars(-WB_ID, -day_of_ice_filled), function(x) gsub(".d", ".yr", x))
```
...
dat <- inner_join(as_tibble(dat.sf), as_tibble(iceCover))
lakes <- dat %>% dplyr::select(WB_ID,AREASQKM,day_of_ice_filled)
...
##  Winter rate
# Waldo et al In Review.  Mean rate from Nov.1 - April 1 = 14.4 +/-16.56
...
preds_2010_lakes %<>% mutate(preds_2010_lakes = (preds_2010_lakes * (365-day_of_ice_filled)) * # apply to open-water days
                               1000000 * AREASQKM, na.rm=TRUE) # 1000000m2 = 1km2.  new variable in mg CO2 d-1
...
ggsave(file = "figures/poly.tiff", width = 15, height = 8, units = "in")
...
ggsave(file = "figures/point.tiff", width = 15, height = 8, units = "in")
```


## Plots of difference in emission rates: 2010 - tmdl: point plots
```{r eval = FALSE}
...
st_write(dat.pt, 'store\\results\\results.shp') ## export lake polys
...
ggsave(file = "figures/pointDiff.tiff", width = 15, height = 8, units = "in")
...
## Plots of difference in emission rates: 2010 - tmdl: point plots, green ramp
```{r eval = FALSE}
# Point plot-------------------
# chl figure 
# better without extreme values
...
p5.col <- p5 + scale_color_gradientn(colors = rainbow(5))
p6.col <- p6 + scale_color_gradientn(colors = rainbow(5))


g <- gridExtra::grid.arrange(p2.col, p3.col, p4.col, p5.col, p6.col, ncol = 3, nrow = 2)

ggsave(file = "figures/pointDiffRainbow.tiff", g, width = 15, height = 8, units = "in")
...
ggsave(file = "figures/pointIncremental.tiff", width = 15, height = 5, units = "in")
```
...
p.ch4 <- ggplot(dplyr::filter(dat.yr.sf.all, (ch42010.lk.yr - ch4TMDLnew.lk.yr)/ch42010.lk.yr > -1), # one value at -10 that distorts scale
  ...
  xlab(expression(Avoided~CH[4]~emissions~per~lake~(metric~tons~yr^{-1}))) +
  theme_bw() + 
  theme(axis.title.y = element_blank())
...
p.n2o <- ggplot(dat.yr.sf.all, 
  ...
  xlab(expression(Avoided~N[2]*O~emissions~per~lake~(kg~yr^{-1}))) +
  theme_bw() + 
  theme(axis.title.y = element_blank())
...
p.co2 <- ggplot(dat.yr.sf.all, 
  ...
  #geom_rug(length = grid::unit(3, units = "mm")) +
  xlab(expression(Avoided~CO[2]~emissions~per~lake~(metric~tons~yr^{-1}))) +
  theme_bw() + 
  theme(axis.title.y = element_blank())
...
cowplot::plot_grid(p2, p3, ncol = 1, nrow = 2, align = "hv")
ggsave(file = "output/figures/NElakesOutputMap.tiff", width = 5, height = 5.5, units = "in")
...
dat.yr.sf.all %>% 
  # select the stuff Elizabeth wantd
