...
plan(multisession) # initiate parralell processing with future package
...
#N = 10
N = 1e4 # 9 minutes with pbreplicate on laptop, 2 minutes with future_replicate



tic()
ch4 <- future_replicate(N, { ## pbreplicate gives a progress bar
  ...
  list(
    ...
    emissions.by.type.ch4,
    preds_2010_lakes %<>% rename(preds_2010_lakes_Mg = preds_2010_lakes), # predict
...
## extract lake specific results
plan(multisession)
tic() # 20 elements: 1.9 sec map, 10.87 sec future_map, weird
poo <- list(ch4[6,], # tmdl scenario.  
            ch4[5,])  %>% # 2010 scenario
  ...
  purrr::map(., function(.x) { # furrr::future_map
    .x %>% 
  ...
  # collapse into single data frame.  data.table implentation of rbind is fast
...
for (i in 1:N) {
  ...
  preds_2010_lakes %<>% 
    mutate(preds_2010_lakes = (preds_2010_lakes * (365-day_of_ice_filled)) * # apply to open-water days
             1e6 * AREASQKM) # 1000000m2 = 1km2.  new variable in mg CO2
  
  preds_tmdl_lakes %<>% 
    mutate(preds_tmdl_lakes = (preds_tmdl_lakes * (365-day_of_ice_filled)) * # apply to open-water days
             1e6 * AREASQKM) # 1000000m2 = 1km2.  new variable in mg CO2
  ...
  preds_2010_lakes %<>% 
    summarize(preds_2010_lakes = sum(preds_2010_lakes / (1e3*1e3*1e3), na.rm = T)) %>%
    mutate(preds_2010_lakes = preds_2010_lakes + co2.from.ch4.2010[i]) 
  
  preds_tmdl_lakes %<>%
    summarize(preds_tmdl_lakes = sum(preds_tmdl_lakes / (1e3*1e3*1e3), na.rm = T)) %>%
...
dat.poly <- right_join( # retains all rows in y, y == chesDat observations
  dplyr::select(nhdSf, COMID), # x is nhd data
  dat %>% mutate(WB_ID = as.integer(WB_ID)), # tibble containing chesDat and day of ice filled
  by = c("COMID" = "WB_ID")) %>% # 
  mutate(WB_ID = as.character(COMID)) %>% # restore WB_ID column for consistency with original data.  convert to character for plotting
  dplyr::select(-COMID) # remove residual COMID column
...
dim(dat.poly) # 4222
class(dat.poly)
plot(dat.poly$Shape) #test, looks good
