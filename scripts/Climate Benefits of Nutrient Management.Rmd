...
ch4 <- pbreplicate(N, { ## pbreplicate gives a progress bar
  ...
  emissions.by.type.ch4 = 
    bind_rows(
      ...
      left_join(
        left_join(
          aggregate(preds_2010_lakes$preds_2010_lakes - preds_tmdl_lakes$preds_tmdl_lakes, 
    ...
    summarize(incr.ch4.mean               = mean(incr.ch4.mean, na.rm = T),
  ...
  emissions.by.type.co2 =
    bind_rows(
      ...
      left_join(
        ...
        mutate(incr.co2.mean = preds_2010_lakes - preds_tmdl_lakes) 
    ) %>% 
    group_by(type, climate.zone) %>% 
    summarize(incr.co2.mean = mean(incr.co2.mean, na.rm = T),
              .groups = 'drop') %>% 
    ungroup 
...
n2o <- pbreplicate(N, {
  ...
  emissions.by.type.n2o =
    ...
    mutate(incr.n2o.mean = preds_2010_lakes - preds_tmdl_lakes) %>% 
    group_by(type, climate.zone) %>% 
    summarize(incr.n2o.mean = mean(incr.n2o.mean, na.rm = T),
              .groups = 'drop') %>% 
    ungroup 
