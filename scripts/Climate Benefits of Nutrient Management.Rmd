...
ice <- nc_open("store/ice.cover.nc") 
...
summary(lon);summary(lat) # lon matches bounding box for watershed, but lat encompasses a larger range.  I must have screwed up CDS query.  This doesn't hurt anything, but may slow things down.  
...
# each list element is a df containing full time series (Jan - March 2019, Nov - Dec. 2019, 
# Jan - March 2020, Nov - Dec 2020) of ice depth for each lake.
# 22 seconds
lakeCtr.l.i <- pblapply(lakeCtr.l, function(x) { # pblapply produces progress bar
  data.frame(ice = nc.get.var.subset.by.axes(ice, "licd", # extract u10 component 
    ...
    mutate(ice = ice * 100, # convert from m to cm:  
           ice.present = ifelse(ice > 1, # thickness > 1 cm
                                1, # yes, ice cover
                                0)) %>% # else, no ice cover
    summarise(day.of.ice = sum(ice.present))

})

lakeCtr.i <- do.call("rbind", lakeCtr.l.i) # 30 seconds
head(lakeCtr.i) # day of ice per row
...
lakeCtr_sf <- lakeCtr.sf %>% 
  ...
  # .shp replaces NA with 0!  used -9999 as flag to screen in GIS.
  # in GIS, convert to .gdb, select all -9999 values, use field calculator
  # to set to 'None', then use 'Fill Missing Values" tool (K (5) nearest neighbors).
  # finally, use spatial join to merge filled values with original file.
...
```{r message = FALSE, fig.cap = "Ice cover duration during the 2019-2020 winter for waterbodies in Chesapeake Bay Watershed model"}
ggplot(states) +
      geom_sf(data = wsb) + # 
  geom_sf(fill = NA) +
...
##  Winter rate
# Waldo et al In Prep.  Mean rate from Nov.1 - April 1 = 14.4
ch4.winter <- 14.4 # mg CH4 m-2 d-1
