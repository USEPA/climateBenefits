...
####  PREDICTIONS
set.seed(42)
ch4 <- pbreplicate(1e4, { #1e4.  pbreplicate gives a progress bar
  ...
  ## THIS IS WHERE WE NEED TO EXPORT LAKE SPECIFIC EMISSIONS REDUCTIONS FROM EACH SIMULATION TO EXTRAPOLATE TO THE MISSISSIPPI RIVER, REPEAT WITH CO2 AND N2O
  
  
  ## Convert and take difference in predictions and sum across all lakes and store CO2 from methanotrophy
  list(
    # CH4 incremental change, summed across all lakes
    incr.ch4 = sum(preds_2010_lakes$preds_2010_lakes - preds_tmdl_lakes$preds_tmdl_lakes, na.rm=TRUE),
    # CO2 produced via methanotrophy under ice in 2010
    methanotrophy.co2.2010 = sum((ch4.winter.truncated * preds_2010_lakes$day_of_ice_filled * 1e6 * preds_2010_lakes$AREASQKM * ch4.prop.methantrophy * (ch4.to.co2) * 44/16), na.rm=TRUE) / (1e3*1e3*1e3), # mg->g, g->kg, kg->Mg. 1 MG = 1 metric ton = 1000kg,
    # co2 produced via methanotrophy under ice in tmdl scenario
    methanotrophy.co2.tmdl =   sum(((ch4.winter.truncated * tmdl.winter) * preds_2010_lakes$day_of_ice_filled * 1e6 * preds_2010_lakes$AREASQKM * ch4.prop.methantrophy * (ch4.to.co2) * 44/16), na.rm=TRUE) / (1e3*1e3*1e3) # mg->g, g->kg, kg->Mg. 1 MG = 1 metric ton = 1000kg
  )
...
co2.from.ch4.2010 = unlist(ch4[2,]) # 1 MG = 1 metric ton = 1000kg
co2.from.ch4.tmdl = unlist(ch4[3,]) # 1 MG = 1 metric ton = 1000kg
...
####  PREDICTIONS
set.seed(42)
diff_co2 <- NULL # empty object to collect results
# using for loop because i need to reference index i.
  ...
  #################################
  #################################
  preds_2010_lakes %<>% mutate(preds_2010_lakes = pCo2(preds_2010_lakes$`dat_2010 %*% coefs`)) # unit conversion: mg CO2 m-2 day-1
  preds_tmdl_lakes %<>% mutate(preds_tmdl_lakes = pCo2(preds_tmdl_lakes$`dat_tmdl %*% coefs`)) # unit conversion: mg CO2 m-2 day-1
  
  # CO2 produced during open-water periods
  preds_2010_lakes %<>% mutate(preds_2010_lakes = (preds_2010_lakes * (365-day_of_ice_filled)) * # apply to open-water days
                                 1e6 * AREASQKM) # 1000000m2 = 1km2.  new variable in mg CO2
  ...
  # mg->g, g->kg, kg->Mg. 1 MG = 1 metric ton = 1000kg
  ######DO WE WANT TO CAPTURE LAKE SPECIFIC RESULTS, OR JUST AGGREGATE ACROSS THE WATERSHED?
  preds_2010_lakes <- preds_2010_lakes %>% 
    summarize(preds_2010_lakes = sum(preds_2010_lakes / (1e3*1e3*1e3))) %>%
                mutate(preds_2010_lakes = preds_2010_lakes + co2.from.ch4.2010[i]) 
  preds_tmdl_lakes <- preds_tmdl_lakes %>%
    summarize(preds_tmdl_lakes = sum(preds_tmdl_lakes / (1e3*1e3*1e3))) %>%
    mutate(preds_tmdl_lakes = preds_tmdl_lakes + co2.from.ch4.tmdl[i])
  
  ## Convert and take difference in predictions and sum across all lakes
  diff_co2[i] <- sum(preds_2010_lakes$preds_2010_lakes - preds_tmdl_lakes$preds_tmdl_lakes, na.rm=TRUE)
...
diff_n2o <- pbreplicate(1e4, { # 1e4
  ...
  preds_2010_lakes %<>% mutate(preds_2010_lakes = pN2o(preds_2010_lakes$`dat_2010 %*% coefs`)) # unit conversion: mg N2O m-2 day-1
  preds_tmdl_lakes %<>% mutate(preds_tmdl_lakes = pN2o(preds_tmdl_lakes$`dat_tmdl %*% coefs`)) # unit conversion: mg N2O m-2 day-1
  
  # open water rates: mg N2O d-1
  preds_2010_lakes %<>% mutate(preds_2010_lakes = (preds_2010_lakes * (365-day_of_ice_filled)) * # apply to open-water days 
