...
# 50 seconds
nhdSf <- st_read(dsn = "../../climateBenefits_jake/climateBenefits/store/NHDPlusV21_National_Seamless_Flattened_Lower48.gdb",
                 layer = "NHDWaterbody") %>%
...
dat.yr <- dat %>%
  ...
  mutate(across(contains("ch4"), function(x) {
    (x * (7*30)) + # emissions during 7 warm months (April 1 - Nov. 1)
      ((ch4.winter * 1000000 * AREASQKM) * # 1000000m2 = 1km2.  mg CH4 d-1
         ((5*30)-day_of_ice_filled))} # 5 months of open-water winter rates, discounted by days of ice)
  )
  ) %>%
...
## object to store extra co2
co2.from.ch4 = tibble()

set.seed(42)
diff_ch4 <- replicate(2, {
  ...
  ## we want a trucated normal such that winter rates don't extend beyond the tails of the summer emission rates
  # set.seed(42)
  ch4.winter.truncated <- rtruncnorm(n=nrow(lakes), a=-1, b=4000, mean=10.56, sd=11.52)
  ...
  ## recover co2 from methantrophy from each iteration of the simulation
  co2.from.ch4 = bind_rows(co2.from.ch4,
                           tibble(co2_from_ch4_2010 = sum((ch4.winter.truncated * preds_2010_lakes$day_of_ice_filled * 1e6 * preds_2010_lakes$AREASQKM * ch4.prop.methantrophy * (ch4.to.co2) * 44/16), na.rm=TRUE),
                                  co2_from_ch4_tmdl = sum(((ch4.winter.truncated * tmdl.winter) * preds_2010_lakes$day_of_ice_filled * 1e6 * preds_2010_lakes$AREASQKM * ch4.prop.methantrophy * (ch4.to.co2) * 44/16), na.rm=TRUE)))
...
set.seed(42)
diff_co2 <- replicate(10000, { #10000
  ...
  preds_2010_lakes %<>% mutate(preds_2010_lakes = (preds_2010_lakes * (365-day_of_ice_filled)) * # apply to open-water days
                                 1000000 * AREASQKM) # 1000000m2 = 1km2.  new variable in mg CO2 d-1
  
  preds_tmdl_lakes %<>% mutate(preds_tmdl_lakes = (preds_tmdl_lakes * (365-day_of_ice_filled)) * # apply to open-water days
                                 1000000 * AREASQKM) # 1000000m2 = 1km2.  new variable in mg CO2 d-1
  
  preds_2010_lakes %<>% mutate(preds_2010_lakes = preds_2010_lakes / (1000*1000*1000)) # mg->g, g->kg, kg->Mg. 1 MG = 1 metric ton = 1000kg
  preds_tmdl_lakes %<>% mutate(preds_tmdl_lakes = preds_tmdl_lakes / (1000*1000*1000)) # mg->g, g->kg, kg->Mg. 1 MG = 1 metric ton = 1000kg
...
diff_n2o <- replicate(10000, {
  ...
  preds_2010_lakes %<>% mutate(preds_2010_lakes = (preds_2010_lakes * (365-day_of_ice_filled)) * # apply to open-water days 
                                 1000000 * AREASQKM, na.rm=TRUE) # 1000000m2 = 1km2.  new variable in mg N2O d-1
...
dat.pt %>% sf::st_drop_geometry(.) %>%
  ...
  mutate(across(contains("Chla"), ~.x*1000)) %>% # convert to ug/L for convenience
...
dat.yr.sf <- dat.yr.sf %>%
  mutate(ch4diff.MT = (ch42010.lk.yr - ch4TMDLnew.lk.yr)/(1000*1000*1000), # mg->g, g->kg, kg->Mg. 1 MG = 1 metric ton = 1000kg)
         co2diff.MT = (co22010.lk.yr - co2TMDLnew.lk.yr)/(1000*1000*1000), # mg->g, g->kg, kg->Mg. 1 MG = 1 metric ton = 1000kg)
         n2odiff.Kg = (n2o2010.lk.yr - n2oTMDLnew.lk.yr)/(1000*1000)) # mg->g, g->kg, 
...
p1 <- ggplot() +
  ...
  geom_sf(data = dat.yr.sf %>% arrange(AREASQKM), # larger dots plot on top of small
          aes(color = (ch42010.lk.yr - ch4TMDLnew.lk.yr)/(1000*1000*1000), # mg->g, g->kg, kg->Mg. 1 MG = 1 metric ton = 1000kg)
              size = AREASQKM)) +
...
p2 <- ggplot() +
  ...
  geom_sf(data = cbb, fill = "white", size = NA) + 
  geom_sf(data = dat.yr.sf %>% arrange(AREASQKM), # larger dots plot on top of small
          aes(color = (co22010.lk.yr - co2TMDLnew.lk.yr)/(1000*1000*1000), # mg->g, g->kg, kg->Mg. 1 MG = 1 metric ton = 1000kg)
              size = AREASQKM)) +
...
p3 <- ggplot() +
  ...
  geom_sf(data = cbb, fill = "white", size = NA) + 
  geom_sf(data = dat.yr.sf %>% arrange(AREASQKM), # larger dots plot on top of small
          aes(color = (n2o2010.lk.yr - n2oTMDLnew.lk.yr)/(1000*1000), #
              size = AREASQKM)) +
...
cnty.increment <- cnty.increment %>%
  mutate(across(contains(".lk.yr"), ~ . / (1000*1000*1000))) %>% # mg->g, g->kg, kg->Mg. 1 MG = 1 metric ton = 1000kg)
...
p.ch4 <- ggplot(dplyr::filter(dat.yr.sf.all, (ch42010.lk.yr - ch4TMDLnew.lk.yr)/ch42010.lk.yr > -1), # one value at -10 that distorts scale
                aes(x = (ch42010.lk.yr - ch4TMDLnew.lk.yr) / (1000*1000*1000))) + # mg->g, g->kg, kg->Mg. 1 MG = 1 metric ton = 1000kg)
...
p.n2o <- ggplot(dat.yr.sf.all, 
                aes(x = (n2o2010.lk.yr - n2oTMDLnew.lk.yr) / (1000*1000))) + # mg->g, g->kg)
  geom_histogram(bins = 75) +
...
p.co2 <- ggplot(dat.yr.sf.all, 
                aes(x = (co22010.lk.yr - co2TMDLnew.lk.yr) / (1000*1000*1000))) + # mg->g, g->kg, kg->Mg. 1 MG = 1 metric ton = 1000kg)
  geom_histogram(bins = 75) +
...
dat.yr.sf.all %>% 
  st_drop_geometry() %>%
  mutate(p.ch4 = (ch42010.lk.yr - ch4TMDLnew.lk.yr) / (1000*1000*1000),
         p.n2o = (n2o2010.lk.yr - n2oTMDLnew.lk.yr) / (1000*1000),
         p.co2 = (co22010.lk.yr - co2TMDLnew.lk.yr) / (1000*1000*1000)) %>%
...
dat.bin <- dat.yr.sf.all %>% 
  st_drop_geometry() %>%
  mutate(p.ch4 = (ch42010.lk.yr - ch4TMDLnew.lk.yr) / (1000*1000*1000),
         p.n2o = (n2o2010.lk.yr - n2oTMDLnew.lk.yr) / (1000*1000*1000),
         p.co2 = (co22010.lk.yr - co2TMDLnew.lk.yr) / (1000*1000*1000),
...
dat.yr.sf.all <- dat.yr.sf.all %>%
  mutate(ch4diff.MT = (ch42010.lk.yr - ch4TMDLnew.lk.yr) / (1000*1000*1000),
         n2odiff.Kg = (n2o2010.lk.yr - n2oTMDLnew.lk.yr) / (1000*1000),
         co2diff.MT = (co22010.lk.yr - co2TMDLnew.lk.yr) / (1000*1000*1000),
...
chl.summary <- dplyr::filter(dat.long, grepl(pattern = "Chl", x = name), value < 0.2) %>%
  mutate(value = value * 1000) %>% # mg -> ug/L
