...
mrb.waterbodies =
  list.files('output/lakes_in_mrb/', pattern = "*.parquet", full.names = T) %>%
...
emissions.by.type =
  left_join(
    left_join(
      readr::read_csv('output/emissions_by_lake_characteristics_ch4.csv', show_col_types = F),
      readr::read_csv('output/emissions_by_lake_characteristics_co2.csv', show_col_types = F),
      by = c('type', 'climate.zone')
    ),
    readr::read_csv('output/emissions_by_lake_characteristics_n2o.csv', show_col_types = F),
    by = c('type', 'climate.zone')
  ) %>%
  rename(subtype = type,
...
for(STATE in mrb.states){
  data %>%
    st_drop_geometry %>%
    filter(stusps == STATE) %>%
    write_parquet(paste0('output/mrb_emissions_reductions_text/mrb_emissions_reductions_text_globalid_', STATE, '.parquet'))
}
