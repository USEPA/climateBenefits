...
# Setup R
The following code can be used to reproduce the results presented in Climate Benefits of Nutrient Management in North America's Largest Estuary, currently under review at Nature-Water.  The code is executed using R version 4.0.3.  Libraries used in this project are controlled by the RENV library.  After cloning this project, run `renv::restore()` to populate the workspace with the appropriate packages and versions.

This code is presented as Rmarkdown document which can be knit to a .html.  Alternatively, individual code chunks can be sent to the console. Below is the configuration used for this project.
...
Lakes and reservoirs are sources of the greenhouse gases (GHG) methane (CH~4~), carbon dioxide (CO~2~), and nitrous oxide (N~2~O).  Emission rates (mass of GHG/unit time/unit area) tend to increase with lake nutrient and chlorophyll concentrations.  Water quality regulations that limit nutrient loading to surface waters could lead to reduced lake nutrient and chl content, thereby indirectly reducing GHG emission rates.  

In this project we combined modeled chlorphyll and nutrient concentrations for ~4000 lakes in the Chesepeake Bay watershed under 1) psuedo current conditions,  and 2) a hypothetical future scenario where water quality regulations have reduced lake nutrient and chlorophyll concentrations.  These estimates were combined with published models (DelSontro et al. 2018) that predict GHG emission rates from lake chlorophyll and nutrient concentration.  The objective of the project is to quantify the aggregate reduction in GHG emissions from the ~4000 lakes in the watershed following the water quality improvement.

# Methods
## Lake Model

The 'Northeast Lakes' model is used to estimate chlorophyll a (ug/L) and total phosphorus (TP; ug/L) for lakes and reservoirs within the Chesapeake Bay watershed under conditions in 2010 and following the implementation of a TMDL policy:

**2010** – represents an estimate of “current” load levels and uses estimated 2010 land uses, animal populations, atmospheric deposition, and point source loads.   

**TMDLnew** - Based on the final Watershed Implementation Plans approved by EPA.

The modeled chlorophyll and nutrient concentrations are reported in ChesLakeLoadsConc.xlsx.  Below the data are read in and the first 6 lines printed.

```{r}
chesDat <- read_excel("store/ChesLakeLoadsConc.xlsx",
                      sheet = "ChesLakeConc")
chesDat %>% 
  select(WB_ID, Chla2010, ChlaTMDLnew, Pvv2010, PvvTMDLnew) %>%
  mutate(across(where(is.numeric), ~.x*1e3)) %>% # *1e3 mg/L to ug/L
  rename("2010 chl" = Chla2010, "TMDL chl" =  ChlaTMDLnew, "2010 TP" = Pvv2010, "TMDL TP" = PvvTMDLnew) %>%
  head()

```


Each waterbody in the lake model output is uniquely identified by a 'WB_ID' value.  These values correspond to 'COMID' values in the NHDPlusV2 dataset, a spatial database of U.S. waterbodies.  NHDPlusV2 waterbody polygons were merged with output from the Northeast Lake model.  The NHDPlusV2 data contains  surface areas for all waterbodies in the Chesapeake Bay watershed.  Below is a map of the lakes and reservoirs in the Chesapeake Bay Watershed.


```{r fig.cap = "Location of 4,222 waterbodies in Chesapeake Bay Watershed model", results='hide', warning = FALSE, message = FALSE}
# Read in lake and reservoir polygons extracted from NHDPlusV2.
dat.sf <- st_read('store\\study_lakes\\study_lakes.gpkg', quiet = TRUE)
...
wsb <- st_read("maps/chesapeakeBayWatershed/Chesapeake_Bay_Watershed_Boundary.shp", quiet = TRUE)

# read in chesapeake bay boundary
# https://data-chesbay.opendata.arcgis.com/search?categories=boundaries

cbb <- st_read("maps/Chesapeake_Bay_Shoreline_Medium_Resolution.shp", quiet = TRUE)
...
##  Ice cover
Ice cover effectively eliminates the exchange of gases between lakes and the atmosphere; therefore GHG emission rates are 0 during periods of ice cover.  Gases can accumulate in the water beneath the ice, however, and subsequently vent to the atmosphere during ice out.  Our modeling accounts for patterns of ice cover across the watershed using ice cover duration data during the winters of 2010 - 2020 derived from the ERA-5 Land data set (https://cds.climate.copernicus.eu/cdsapp#!/dataset/reanalysis-era5-land?tab=overview) and spatially joined to the centroid of the NHD polygon for each waterbody in the watershed. Mean annual days of ice cover were calculated for each lake.
```{r ice_cover, warning = FALSE, message = FALSE, results = FALSE, fig.cap = "Mean ice cover duration during the 2010-2020 winters for waterbodies in Chesapeake Bay Watershed model"}
# read in ice cover data--------------------
iceCover <- st_read("output/iceCover.gdb", layer = "iceCoverFill", quiet = TRUE) %>%
...
# map of mean annual ice cover duration
ggplot(states) +
  geom_sf(data = wsb) + # 
  geom_sf(fill = NA) +
...
# GHG model
DelSontro et al. (2018) present linear regression models based on literature reports of chlorophyll a concentration (ug/L), TP concentration (ug/L) and areal GHG emission rates.  The models were created on the log-log scale with a small positive offset to accommodate negative areal emission rates reported in the literature.  Emission rates were standardized to the following units: mg CH~4~-C m^-2^ day^-1^, mg CO~2~-C m^-2^ day^-1^, or mg N~2~O-N m^-2^ day^-1^.  Predictions are back transformed to linear space and converted from mg of C or N, to mass of CH~4~, CO~2~, or N~2~O via helper functions. Below we read in the model objects and define the helper functions.
...
We use the DelSontro et al. (2018) models to predict areal GHG emission rates (mg GHG m^-2^ day^-1^) for each waterbody under the 2010 and TMDL scenarios.  Uncertainty in emission rate predictions for each waterbody was estimated by randomly sampling from the distribution of the model coefficients using their covariance matrix to accommodate correlation within draws. We iterate this with 10,000 random draws.  Full code is provided below for CH~4~, CO~2~, and N~2~O.  Code output is  suppressed in the Rmarkdown document, but results are presented in the `Incremental Change` section.
...
# The simulation below is fairly time consuming. After the simulation has completed,
# code at the bottom of this chunk will store the results locally.  If the objects
# are stored locally, the cptional code below will will load these objects into the
# workspace, rather than rerunning the simulations.  This is done to speed up the
# knitting process.
# if(file.exists("output/diff_co2.rda")) {
#   load("output/diff_co2.rda")
# }
# if(file.exists("output/emissions.by.type.co2.rda")) {
#   load("output/emissions.by.type.co2.rda")
# }
# if(file.exists("output/co2.by.lake.rda")) {
#   load("output/co2.by.lake.rda")
# }
...
toc()
# 
# save(diff_co2, file = "output/diff_co2.rda")
# save(emissions.by.type.co2, file = "output/emissions.by.type.co2.rda")
# save(co2.by.lake, file = "output/co2.by.lake.rda")
```
...
#### Incremental Change
The table below contains the difference in GHG emissions between the 2010 and TMDL scenarios.  The mean, upper 95% confidence limit, and lower 95% confidence limit are derived from the 10,000 realizations of the simulation for each gas.
```{r warning=FALSE, message=FALSE}

## export emissions by climate zone and type
# these data are used in the MARB extrapolation
...
kr_simulations %>%
  kbl(col.names = rep(c("mean", "upper 95", "lower 5"), 3),
      caption = "Incremental GHG (metric tons) calculated from simulation") %>%
...
#Some spatial patterns are better shown on a map when each waterbody is sybmolized as a point rather than a polygon.  
#Here we calculate and plot the centroid of each waterbody.
...
# Reshape data into long for CDF plots
...
## Figure 4: areal N~2~O, CH~4~, CO~2~ reductions spatial
```{r}
# CH4 figure
p4 <- ggplot() +
  geom_sf(data = wsb) +
...
dat.cum.emissions %>%
  ...
  scale_color_manual(values = c('#58AB41', '#CDA4DE', '#FAC80A'),
                     labels = c(expression(CO[2]), expression(N[2]*O), expression(CH[4])),
                     breaks = c('co2diffMg', 'n2odiffKg', 'ch4diffMg')) +
...
## export
ggsave(file = "output/figures/Figure5.tiff", width = 5, height = 3.5, units = "in")
ggsave('output/figures/Figure5.svg', width = 5, height = 3.5)

```

## Supplemental Figure 1
Supplemental Figure 1: The difference in pollutant concentrations and areal emissions rates from baseline to TMDL by latitude. Top row  : relationship between latitude and difference in the concentration (ug/L) of total phosphorus and chlorophyll-a between Baseline and TMDL scenarios. Bottom row: relationship between latitude and difference in areal emission rates (g m^{-2} y^{-1}) of methane (CH~4~), carbon dioxide (CO~2~), nitrous oxide (N~2~O) between Baseline and TMDL scenarios. Positive values on the y-axis indicate lower emission rates (top row) or concentrations (bottom row) under the TMDL scenario.
```{r supplementalFig1}
...
```{r supplementalFig2, warning=FALSE, fig.cap="Supplemental Figure 2: The relationship between TMDL policy effect on areal methane (CH4) emission rates and chlorophyll-a.  Observations with y-axis values less than -25 (n=2) or greater than 100 (n=5) are omitted to better highlight the space where 99.8% of the observations lie."}
...
```{r supplementalFig3, fig.cap="Supplemental Figure 3: Number of days per year that waterbodies are ice covered." }
ggplot() +
  geom_sf(data = wsb) +
...
```{r supplementalFig4, fig.cap="Supplemental Figure 4: TMDL policy affect on areal CO2 emission rates by waterbody size."}
...
```{r supplementalFig5, fig.cap="Supplemental Figure 5: Distribution of lake and reservoir surface areas in the Chesapeake Bay watershed." }
...
# Manuscript Stats
Below is a collection of code that corresponds to numeric results reported in the manuscript.  
