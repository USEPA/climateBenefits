...
setwd(rprojroot::find_rstudio_root_file()) #force wd within a chunk
...
#                       sheet = "ChesLakeConc")
chesDat <- readxl::read_excel("../store/ChesLakeLoadsConc.xlsx", sheet = "ChesLakeConc") #hayley's path

#spatial data - needed for surface area predictor.
# 50 seconds
# dsn = paste0(localPath, 
#                                "/Environmental Protection Agency (EPA)/",
#                                "Moore, Chris - Climate benefits of nutrient management/",
#                                "analysis/store/",
...
mod.ch4 <- readRDS("models/pCh4.rds") #Hayley's path
...
mod.co2 <- readRDS(paste0("models/pCo2.rds")) #Hayley's path
...
mod.n2o <- readRDS(paste0("models/pN2o.rds")) #Hayley's path
...
simulated_total_reduction_CH4<-
  replicate(1000, sim_cov(mean_names=c("CH4_pred_2010","CH4_pred_TMDL"), 
...
simulated_total_reduction_CO2<-
  replicate(1000, sim_cov(mean_names=c("CO2_pred_2010","CO2_pred_TMDL"),
...
simulated_total_reduction_N2O<-
  replicate(1000, sim_cov(mean_names=c("N2O_pred_2010","N2O_pred_TMDL"),
...
```{r CH4 boot, fig.height=9, cache=T}

set.seed(0)
total_reduction_CH4_withbootstrap<-
  replicate(1000, sim_cov_bootstrap(dat=dat.sf, n=nrow(dat.sf), 
...
```{r CO2 boot, fig.height=9, cache=T}
set.seed(0)
total_reduction_CO2_withbootstrap<-
  replicate(1000, sim_cov_bootstrap(dat=use_co2_dat, n=nrow(use_co2_dat), 
...
```{r N2O boot, fig.height=9, cache=T}
set.seed(0)
total_reduction_N2O_withbootstrap<-
  replicate(1000, sim_cov_bootstrap(dat=use_n2o_dat, n=nrow(use_n2o_dat), 
